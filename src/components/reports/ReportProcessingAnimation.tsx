...
const ReportProcessingAnimation = ({ 
  ...
  const [stage, setStage] = useState<'encoding' | 'anonymizing' | 'complete'>('encoding');
      ...
      {stage === 'complete' ? (
        ...
        <>
          <div className="relative w-16 h-16 mb-2">
            <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-500 ${stage === 'encoding' ? 'opacity-100' : 'opacity-0'}`}>
              <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center animate-pulse">
                <FileText className="h-7 w-7 text-blue-600" />
              </div>
            </div>
            <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-500 ${stage === 'anonymizing' ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <h3 className="font-medium mb-1">
            {stage === 'encoding' ? 'Encoding personal information...' : 'Anonymizing report data...'}
          </h3>
          
          <Progress value={progress} className="w-full h-2" indicatorColor={stage === 'encoding' ? '#3b82f6' : '#22c55e'} />
          
          <p className="text-xs text-gray-500 mt-1">Processing report securely</p>
        </>
      )}
    </div>
  );
