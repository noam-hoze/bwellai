...
const generateSleepData = (date: Date, viewType: string): DayData[] | WeekData[] | MonthData[] => {
  if (viewType === "day") {
    return [
      {
        name: "Light Sleep",
...
const SleepChart: React.FC<SleepChartProps> = ({ date, viewType }) => {
  const data = generateSleepData(date, viewType);
  ...
  if (viewType === "day") {
    ...
    const lightSleepActual = dayData.find(item => item.name === "Light Sleep")?.value || 0;
    const deepSleepActual = dayData.find(item => item.name === "Deep Sleep")?.value || 0;
    const remSleepActual = dayData.find(item => item.name === "REM")?.value || 0;
    ...
    const sleepStages = [
      { name: "Light Sleep", value: lightSleepActual, ideal: idealLightSleep, color: "#FEDF89" },
      { name: "Deep Sleep", value: deepSleepActual, ideal: idealDeepSleep, color: "#53A15E" },
      { name: "REM Sleep", value: remSleepActual, ideal: idealRemSleep, color: "#0EA5E9" }
    ];
    
    const dominantStage = [...sleepStages].sort((a, b) => b.value - a.value)[0];
    const percentage = Math.round((dominantStage.value / dominantStage.ideal) * 100);
        ...
        >
          ...
          <XAxis 
            ...
            tickFormatter={(value, index) => {
              if (viewType === "week" && typeof index === "number" && index < data.length) {
          ...
          <Tooltip 
            formatter={(value, name) => {
              const formattedName = name === "light" ? "Light Sleep" :
