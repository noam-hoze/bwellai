  ...
  return (
    <div className="space-y-6">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    id={`schedule-${slot.id}`}
                    checked={goalData.schedule[slot.id as keyof typeof goalData.schedule]}
                    onCheckedChange={(checked) => 
                      handleScheduleChange(slot.id as 'morning' | 'afternoon' | 'evening', checked)
                    }
                  />
                  <Label htmlFor={`schedule-${slot.id}`} className="text-sm">
                    {goalData.schedule[slot.id as keyof typeof goalData.schedule] ? "Enabled" : "Disabled"}
                  </Label>
                </div>
              </div>
            </CardContent>
      ...
      <div className="mt-8">
        ...
        <RadioGroup 
          value={goalData.duration.toString()} 
