...
const GoalReviewStep = ({ goalData }: GoalReviewStepProps) => {
  ...
  const endDate = goalData.duration ? addDays(startDate, goalData.duration) : addDays(startDate, 28);
  ...
  // Default to "Back Pain Relief" if type is not defined
  const goalType = goalData.type || "Back Pain Relief";

  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold mb-2">{goalType} Management Goal</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">{getScheduleText() || "Not specified"}</p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              ...
              <span className="block text-xs text-gray-500">({goalData.duration || 28} days)</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-wellness-light-green p-4 rounded-lg">
        ...
        <p className="mt-1 text-sm">
          This {goalType.toLowerCase()} management goal will help you reduce pain through
          guided exercises. You'll track your progress daily for {goalData.duration || 28} days and should start seeing results within 
          {(goalData.duration || 28) <= 14 ? " 1-2 weeks" : (goalData.duration || 28) <= 30 ? " 2-4 weeks" : " 4-8 weeks"}.
        </p>
      </div>
    </div>
  );
