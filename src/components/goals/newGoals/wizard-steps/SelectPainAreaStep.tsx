...
const SelectPainAreaStep: React.FC<SelectPainAreaStepProps> = ({ 
  ...
  const { data: userGoalDetails, isLoading: userGoalDetailsIsLoading } =
      useUserGoalDetails();
  const handleSelectArea = (type: string, id: number, name: string) => {
    updateGoalData({ type: type , goalId: id, name: name });
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {userGoalDetails?.map((goal) => (
          <Card 
            ...
            onClick={() => handleSelectArea(goal.goalType, goal.id, goal.name)}
          >
