...
} from '@/models/constant/report-data/body-profile-data';
...
const HealthNavigator = ({ getProfileIsData, userPreviousData }) => {
  ...
  const getStatusBadge = (status: 'normal' | 'caution' | 'warning' | 'abnormal' | 'high') => {
    switch (status) {
      case 'normal':
        return (
        ...
        <div className="max-w-4xl mx-auto bg-gray-50 p-4 rounded-lg">
          ...
          <div className="relative flex flex-col md:flex-row items-start">
            <div className="w-full flex items-center justify-center overflow-hidden">
              ...
              {(getProfileIsData?.gender === 'female' || !getProfileIsData?.gender) && (
              ...
              {formatedData?.[activeOrgan] ? (
                      ...
                      <div className="space-y-3">
                        {formatedData2 &&
                          healthMetrics?.[activeOrgan]?.map((stat, index) => {
                            ...
                            return formatedData2?.[stat?.metric?.toUpperCase()] ? (
                              <div key={index} className="flex items-start">
                                <div className="mr-2 mt-1">{stat?.icon}</div>
                                <div>
                                  <div className="flex items-baseline">
                                    <span className="text-lg font-bold text-gray-900">
                                      {formatedData2?.[stat?.metric?.toUpperCase()]?.value ||
                                        stat?.noDataMessage}
                                    </span>
                                    {formatedData2?.[stat?.metric?.toUpperCase()]?.unit && (
                                      <span className="ml-1 text-sm text-gray-600">
                                        {formatedData2?.[stat?.metric?.toUpperCase()].unit}
                                      </span>
                                    )}
                                    {getStatusBadge(
                                      formatedData2?.[stat?.metric?.toUpperCase()]?.signalText,
                                    )}
                                  </div>
                              ...
                              <div key={index} className="flex items-start">
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-gray-700">{stat?.metric}</div>
                                  <div
                    ...
                    <div className="p-3">
                      ...
                      <div className="space-y-3">
                        {organData?.[activeOrgan]?.stats?.map((stat, index) => {
                          const isShowData = healthMetricsArray?.[activeOrgan]?.metric?.includes(
                            stat.label,
                          );
                          ...
                          return isShowData ? (
                            <div key={index} className="flex items-start">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-gray-700">{stat.linkText}</div>
                              </div>
                            </div>
                          ) : (
                            <></>
                ...
                <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200 text-center">
                  ...
                  <p className="text-gray-600 mb-4">
                    Click on different parts of the body model to view detailed health metrics and
                    insights.
                  </p>
